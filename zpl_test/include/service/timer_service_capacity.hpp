...
class TimerServiceCapacity{
    ...
    public:
        TimerServiceCapacity():_capacity(64), _max_capacity(_capacity * 3), _timeout_per_batch(2),
            _timer_service(chrono::milliseconds(1000), _capacity) ,_result(false)
        {
        }
                ...
                _timer_service.start();
                {
                    ...
                    for(const uint32_t& timeout:timeout_list){
                        for(size_t i = 0; i< batch_size;i++){
                            timer_id_type timer_id = _timer_service.setTimer(chrono::milliseconds(timeout * 1000), session_id);
    ...
    private:
        void onTimeoutEvent(list < AppMessage >& event_list){
            for(const AppMessage& msg:event_list){
                ...
                //create new timer
                timer_id_type new_timer_id = _timer_service.setTimer(chrono::milliseconds(_timeout_per_batch * 1000), 1);
