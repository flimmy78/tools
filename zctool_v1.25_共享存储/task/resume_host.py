...
class ResumeHostTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler,
        ...
        BaseTask.__init__(self, task_type, RequestDefine.resume_host,
        ...
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.resume_host, result_fail,
                             self.onStartFail)        
        self.addTransferRule(state_initial, AppMessage.EVENT,
        ...
        self.addTransferRule(stTransport, AppMessage.RESPONSE,
                             RequestDefine.resume_host, result_success,
    ...
    def invokeSession(self, session):
        ...
        logging.info("[%08X]request resume_host to control server '%s'"%
    ...
    def onStartSuccess(self, msg, session):
        ...
        logging.info("[%08X]resume_host started"%
                       (session.session_id))
        self.setTimer(session, self.operate_timeout)

    def onStartFail(self, msg, session):
        self.clearTimer(session)
        logging.error("[%08X]resume_host fail due to remote service"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onStartTimeout(self, msg, session):
        logging.error("[%08X]resume_host timeout"%
                  (session.session_id))
    ...
    def onTransportSuccess(self, msg, session):
        ...
        param = self.case_manager.getParam()
        logging.info("[%08X]resume_host success"%
                  (session.session_id))
