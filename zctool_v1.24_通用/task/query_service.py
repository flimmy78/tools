...
class QueryServiceTask(BaseTask):
    ...
    def onQuerySuccess(self, msg, session):
        ...
        version = msg.getStringArray(ParamKeyDefine.version)
        server = msg.getStringArray(ParamKeyDefine.server)
        #disk_type =msg.getUIntArray(ParamKeyDefine.disk_type)
        count = len(name)
        ...
        #title = ['Service Name','IP','Port','Status','version','server','disk_type']
        title = ['Service Name','IP','Port','Status','version','server']
        print_test_result(title,name,ip,port,newstatus,version,server)
        #print_test_result(title,name,ip,port,newstatus,version,server,disk_type)
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query service FAIL"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query service TIMEOUT"%
                  (session.session_id))
