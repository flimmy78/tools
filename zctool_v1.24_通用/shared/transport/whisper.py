...
class Whisper(object):
    ...
    def __init__(self, listen_ip, data_channel, tmp_path, logger_name):        
        ...
        self.invoker = TimedInvoker(self.handleScheduleCheck, self.check_interval)
        self.observer = None
        ...
        self.write_task = {}
        self.read_task = {}
        self.write_allocated = {}
        self.read_allocated = {}
        self.min_write_id = 1
        self.max_write_id = self.min_write_id + self.max_task
        self.min_read_id = self.max_write_id
        self.max_read_id = self.min_read_id + self.max_task
        for i in range(self.max_task):
            self.write_task[self.min_write_id + i] = WhisperWriteTask(self.min_write_id + i, self)
            self.write_allocated[self.min_write_id + i] = False
            self.read_task[self.min_read_id + i ] = WhisperReadTask(self.min_read_id + i, self)
            self.read_allocated[self.min_read_id + i] = False

        self.write_lock = threading.RLock()
        self.read_lock = threading.RLock()
        self.report_counter = self.report_interval / self.check_interval
...
##                entry.average_length, entry.total_length))
    ...
    def allocateWriteTask(self, file_id, remote_ip, remote_port):
        ...
        with self.write_lock:        
            for task_id in range(self.min_write_id, self.max_write_id):
                if not self.write_allocated[task_id]:
                    ##unallocated
                    self.write_allocated[task_id] = True
                    ##write client
                    self.write_task[task_id].allocate(file_id, filename, False,
    ...
    def startWriteTask(self, task_id):
        if (task_id >= self.max_write_id) or (task_id < self.min_write_id):
            self.logger.error("<Whisper>start write task fail, invalid id '%d'"%(
                task_id))
            return False
        task = self.write_task[task_id]
    ...
    def allocateReadTask(self, remote_file_id, remote_ip, remote_port):
        ...
        with self.read_lock:        
            for task_id in range(self.min_read_id, self.max_read_id):
                if not self.read_allocated[task_id]:
                    ##unallocated
                    self.read_allocated[task_id] = True
                    task = self.read_task[task_id]
                    ##read client
                    task.allocate(file_id, filename, False,
                                  remote_ip, remote_port)
    ...
    def startReadTask(self, task_id):
        if (task_id >= self.max_read_id) or (task_id < self.min_read_id):
        ...
        with self.write_lock:
            for task_id in range(self.min_write_id, self.max_write_id):
                if not self.write_allocated[task_id]:
                    ...
                    task = self.write_task[task_id]
                    ##allocate write task at server
                    task.allocate(file_id, filename, True,
                    ...
                    if not task.createReceiver():
                        self.write_allocated[task_id] = False
                        task.reset()
                        self.detachFile(file_id)
    ...
    def handleWriteAck(self, command, sender_ip, sender_port):
        ...
        with self.write_lock:
            ...
            task.startTransport()
            ##notify start
            self.onTaskStart(task_id, TaskTypeEnum.write)
            return True

    def handleReadRequest(self, command, sender_ip, sender_port):
        ##request read at server
        ...
        with self.read_lock:
            for task_id in range(self.min_read_id, self.max_read_id):
                if not self.read_allocated[task_id]:
                    ##task id available  
                    task = self.read_task[task_id]
                    task.allocate(command.file_id, filename, True,
                                  sender_ip, sender_port)
                    task.setRemoteTask(command.receiver)
    ...
    def handleData(self, command, sender_ip, sender_port):
        task_id = command.receiver
        if (task_id >= self.min_write_id) and (task_id < self.max_write_id):
            ##write task
            if not self.write_allocated[task_id]:
                self.logger.error("<Whisper>handle data fail, invalid write task %d"%(
                    task_id))
                return False
            task = self.write_task[task_id]
        elif (task_id >= self.min_read_id) and (task_id < self.max_read_id):
            ##read task
            if not self.read_allocated[task_id]:
                self.logger.error("<Whisper>handle data fail, invalid read task %d"%(
                    task_id))
                return False
            task = self.read_task[task_id]
    ...
    def handleDataAck(self, command, sender_ip, sender_port):
        ...
        if (task_id >= self.min_write_id) and (task_id < self.max_write_id):
            ##write task
            if not self.write_allocated[task_id]:
                self.logger.error("<Whisper>handle data ack fail, invalid write task %d"%(
                    task_id))
                return False
            task = self.write_task[task_id]
        elif (task_id >= self.min_read_id) and (task_id < self.max_read_id):
            ##read task
            if not self.read_allocated[task_id]:
                self.logger.error("<Whisper>handle data ack fail, invalid read task %d"%(
                    task_id))
                return False
            task = self.read_task[task_id]
    ...
    def handleFinish(self, command, sender_ip, sender_port):
        ...
        if (task_id >= self.min_write_id) and (task_id < self.max_write_id):
            ##write task
            if not self.write_allocated[task_id]:
                self.logger.error("<Whisper>handle finish fail, invalid write task %d"%(
                    task_id))
                return False
            task = self.write_task[task_id]
        elif (task_id >= self.min_read_id) and (task_id < self.max_read_id):
            ##read task
            if not self.read_allocated[task_id]:
                self.logger.error("<Whisper>handle finish fail, invalid read task %d"%(
                    task_id))
                return False
            task = self.read_task[task_id]
...
##            task.reset()
    ...
    def handleFinishAck(self, command, sender_ip, sender_port):
        ...
        if (task_id >= self.min_write_id) and (task_id < self.max_write_id):
            ##write task
            if not self.write_allocated[task_id]:
                self.logger.error("<Whisper>handle finish ack fail, invalid write task %d"%(
                    task_id))
                return False
            task = self.write_task[task_id]
        elif (task_id >= self.min_read_id) and (task_id < self.max_read_id):
            ##read task
            if not self.read_allocated[task_id]:
                self.logger.error("<Whisper>handle finish ack fail, invalid read task %d"%(
                    task_id))
                return False
            task = self.read_task[task_id]
...
##                task_id))
    ...
    def handleScheduleCheck(self):
        ...
        need_report = False
        if self.check_counter%(self.report_counter) == 0:
            need_report = True
            self.check_counter = 0
        ...
        with self.write_lock:
            deallocate_list = []
            for task_id in self.write_task.keys():
                if self.write_allocated[task_id]:
                    task = self.write_task[task_id]
                    task.check()
                    if task.isFinished():
                        if task.isSuccess():
                            event_list.append(self.createSuccessEvent(task))
                        else:
                            event_list.append(self.createFailEvent(task))
                        deallocate_list.append(task_id)
                    elif need_report:
                        ##on progress
                        event_list.append(self.createProgressEvent(task))
                        
            if 0 != len(deallocate_list):
                for task_id in deallocate_list:
                    self.write_allocated[task_id] = False
                    self.write_task[task_id].reset()
        ##check read task
        with self.read_lock:
            deallocate_list = []
            for task_id in self.read_task.keys():
                if self.read_allocated[task_id]:
                    task = self.read_task[task_id]
                    task.check()
                    if task.isFinished():
                        if task.isSuccess():
            ...
            if 0 != len(deallocate_list):
                for task_id in deallocate_list:
                    self.read_allocated[task_id] = False
                    self.read_task[task_id].reset()
                    
        if 0 != len(event_list):
            ##notify
            with self.notify_lock:
                self.notify_queue.extend(event_list)
    ...
    def createProgressEvent(self, task):
        event = WhisperEvent()
        event.event_type = WhisperEvent.progress
        event.task_id = task.getTaskID()
        event.task_type = task.getTaskType()
        event.current = task.getProcessed()
        event.total = task.getTotal()
        event.percent = task.getProgress()
        event.speed = task.getSpeed()
        return event

    def createSuccessEvent(self, task):
        event = WhisperEvent()
        event.event_type = WhisperEvent.success
        event.task_id = task.getTaskID()
        event.task_type = task.getTaskType()
        if task.isWriteTask():
            event.file_id = task.getRemoteFile()
        else:
            ##read task, local file
            event.file_id = task.getFileID()
        return event

    def createFailEvent(self, task):
        event = WhisperEvent()
