...
class QueryStorageDeviceTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler,
        ...
        BaseTask.__init__(self, task_type, RequestDefine.query_storage_device,
                          messsage_handler, logger_name)
        
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.query_storage_device, result_success,
                             self.onQuerySuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.query_storage_device, result_fail,
                             self.onQueryFail)
    ...
    def onQuerySuccess(self, msg, session):
        ...
        title.append("target")
        value.append(msg.getStringArray(ParamKeyDefine.target))
        title.append("status")
        value.append(msg.getUIntArray(ParamKeyDefine.status))        
        title.append("available")
        value.append(msg.getUIntArray(ParamKeyDefine.available))
        title.append("disk_used")
        value.append(msg.getUIntArray(ParamKeyDefine.disk_used))
        title.append("disk_volume")
        value.append(msg.getUIntArray(ParamKeyDefine.disk_volume))
        
        self.info("[%08X]query storage device success" % (session.session_id))
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query storage device fail"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query storage device tiemout"%
                  (session.session_id))
