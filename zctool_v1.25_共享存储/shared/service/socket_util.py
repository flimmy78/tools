...
def convertNetmaskToSize(netmask):
    if netmask>32:
        raise Exception("wrong value of netmask, netmask '%s'" % netmask)
    
    _int_netmask = 0
    
    for _i in xrange(0, netmask):
        _int_netmask += 1 << _i
        
    _remain_bits = 32 - netmask
    for _i in xrange(0, _remain_bits):
        _int_netmask = _int_netmask << 1
        
    return ((~_int_netmask) & 0xffffffff)+1
