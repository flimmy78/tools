...
print os.path.join(os.getcwd(),"shared")

#from help import All_help
...
class ZhiCloudMgtCmd(Cmd):
    ...
    def do_quit(self,argc):
        ...
        try:
            ...
            print "Execute fail, goto except now. Error Info:%s" % str(e) 
            print str(e)
    ...
    def do_query_service_type(self,argc):
        """
        ********** query_service_type param ***********
         None.
        ***********************************************
        eg:query_service_type
        """
        try:
            global service
            global finished_event
            ...
            finished_event = threading.Event()
            if service != None:
                service.case_manager.addTestCase(TestCase("query_service_type", query_service_type))
            ...
            print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_query_service_group(self,argc):
        """
        query service group,include control service,node client,storage service.
        *************query_service_type param***************
        none
        ***********************************************
    eg:query_service_group -t 3
        """
        argc=argc.split()
        if param_image(param_query_service_group,argc)==CHECK_PASS:
            try:
               global service
               global finished_event
               finished_event.clear()
               ...
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("query_service_group", query_service_group))
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   enviro["type"]=int(param_query_service_group['-t']) 
                   service.beginTest()

                   finished_event.wait()
                   
                   finished_event.clear()
                   service.case_manager.enviro.clear()
                   #test result:
                   if service.case_manager.test_result == CMDRUN_PASS:
                       print "Execute command success."
                   else:
    ...
    def do_enable_service(self,argc):
        """
        enable service,include control service,node client,storage service.
        *************enable service param***************
         -t(*) : type of query:
             2 : control service.
             3 : node client.
             4 : storage service.
        ***********************************************
        """
        argc=argc.split()
        if param_image(param_enable_service,argc)==CHECK_PASS:
            try:
               global service
               global finished_event
               finished_event.clear()
               ...
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("enable_service", enable_service))
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   enviro["target"]=param_enable_service['-t']
                   service.beginTest()
               ...
               print "Execute fail, goto except now. Error Info:%s" % str(e)
               print str(e)
    def do_disable_service(self,argc):
        """
        disable service,include control service,node client,storage service.
        *************disable service param***************
         -t(*) : type of query:
             2 : control service.
             3 : node client.
             4 : storage service.
        ***********************************************
        """
        argc=argc.split()
        if param_image(param_disable_service,argc)==CHECK_PASS:
            try:
               global service
               global finished_event
               finished_event.clear()
               ...
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("disable_service", disable_service))
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   enviro["target"]=param_disable_service['-t']
                   service.beginTest()
    ...
    def do_query_compute_pool(self,argc):
        ...
        ***********************************************
        ...
        try:
            ...
            print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_query_compute_pool_detail(self,argc):
        argc = argc.split()
        if param_image(param_query_compute_pool_detail,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_compute_pool_detail", query_compute_pool_detail))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["uuid"] = param_query_compute_pool_detail['-id']
        ...
        if param_image(param_create_compute_pool,argc)==CHECK_PASS:
            try:
               ...
               if service != None:               
                   ...
                   enviro["path"] =param_create_compute_pool['-path']
                   enviro["crypt"] =param_create_compute_pool['-crypt']
                   #enviro["mode"] =param_create_compute_pool['-mode']
                   if param_create_compute_pool['-qos'] == '1':
                        enviro["auto_qos"] = True
                   else:
                        enviro["auto_qos"] = False
		   if param_create_compute_pool['-thin'] == '1':
                        enviro["thin"] = True
                   else:
                        enviro["thin"] = False	
		   if param_create_compute_pool['-ha'] == '1':
                        enviro["high_available"] = True
                   else:
                        enviro["high_available"] = False	                    
                   enviro["control_server"] = service.zc_node_name
                   service.beginTest()
                ...
                print "Execute fail, goto except now.",e
        ...
        if param_image(param_modify_compute_pool,argc)==CHECK_PASS:
            try:
               ...
               if service != None:               
                   ...
                   enviro["path"] = param_modify_compute_pool['-path']
                   enviro["crypt"] = param_modify_compute_pool['-crypt']
                   enviro["option"] = param_modify_compute_pool['-op']
		   if param_modify_compute_pool['-qos'] == '1':
                        enviro["auto_qos"] = True
                   else:
                        enviro["auto_qos"] = False
		   if param_modify_compute_pool['-thin'] == '1':
                        enviro["thin"] = True
                   else:
                        enviro["thin"] = False
                   if param_modify_compute_pool['-ha'] == '1':
                        enviro["high_available"] = True
                   else:
                        enviro["high_available"] = False
                   enviro["control_server"] = service.zc_node_name
                   #if param_modify_compute_pool['-op'] == '1':
                       #enviro["modify_pool"] = True
                   #else:
                       #enviro["modify_pool"] = False
        ...
        if param_image(param_query_compute_resource,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["pool"] = param_query_compute_resource['-p']
                    #start Test
                    service.beginTest()
        ...
        if param_image(param_upload_iso_image,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #enviro["target"] = param_upload_iso_image['-t']
                    enviro["name"] = param_upload_iso_image['-name']
                    enviro["description"] = param_upload_iso_image['-des']
                    enviro["target"] = param_upload_iso_image['-path']
                    enviro["disk_type"] = param_upload_iso_image['-dt']
                    enviro["uuid"] = param_upload_iso_image['-id']
                    #begin test.
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
        ...
        *************************************************
        ...
        if param_image(param_delete_iso_image,argc) == CHECK_PASS:
            try:
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_query_whisper(self,argc):
        try:
            global service
            global finished_event
            finished_event.clear()
            ...
            if service != None:
                #add TestCase to CaseManager
                service.case_manager.clearTestCase()
                service.case_manager.addTestCase(TestCase("query_whisper", query_whisper))
                #parameter initail and begin test.
                enviro = service.case_manager.getParam()
                enviro["control_server"] = service.zc_node_name
                service.beginTest()
    ...
    def do_insert_media(self,argc):
        argc = argc.split()
        if param_image(param_insert_media,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("insert_media", insert_media))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_insert_media['-id']
                    enviro["image"] = param_insert_media['-image']
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
                    finished_event.clear()
    ...
    def do_change_media(self,argc):
        argc = argc.split()
        if param_image(param_change_media,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("change_media", change_media))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_change_media['-id']
                    enviro["image"] = param_change_media['-image']
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
                    finished_event.clear()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_eject_media(self,argc):
        argc = argc.split()
        if param_image(param_eject_media,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("eject_media", eject_media))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_eject_media['-id']
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
                    finished_event.clear()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_migrate_host(self,argc):
        argc = argc.split()
        if param_image(param_migrate_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("migrate_host", migrate_host))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["host"] = param_migrate_host['-id']
                    enviro["target"] = param_migrate_host['-t']
                    enviro["type"] = param_migrate_host['-type']
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
                    finished_event.clear()
        ...
        if param_image(param_query_host,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro["range"] = param_query_host['-rg'] 
                    enviro["target"] = param_query_host['-tg']         
                    #optional parameters
                ...
                print "Execute fail, goto except now,%s" % str(e)
        ...
        if param_image(param_create_host,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    service.case_manager.clearTestCase()
                    enviro = service.case_manager.getParam()
                    #print int(param_create_host['-count'])
                    for i in range(int(param_create_host['-count'])):
                        ...
                        enviro["io"] = param_create_host['-io']
                        enviro["priority"] = param_create_host['-pry']
                        ...
                        if param_create_host['-bk'] == '1':
                            enviro["backup"] = True
                        else:
                            enviro["backup"] = False
                        if param_create_host['-usb'] == '1':
                            enviro["usb"] = True
                        else:
                            enviro["usb"] = False
                        ...
                        enviro["path"] = param_create_host['-path']
                        enviro["crypt"] = param_create_host['-crypt']
    ...
    def do_reset_host(self,argc):
        """
        reset_host:reset host.
        *************** reset_host param *****************
         -id(*) : host id.
        *************************************************
        eg:reset_host -id 6f8d9ae7-fe2f-4869-838c-e591254e6f9b
        """
        argc = argc.split()
        if param_image(param_reset_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("reset_host", reset_host))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["host"] = param_reset_host['-id']
                    service.beginTest()
    ...
    def do_modify_host(self,argc):
        ...
        *************** modify_host param ***************
         ...
         -op :  option select:auto start,local_backup,usb.value:0/1 0:false 1:true,default:'0'.
        ...
        if param_image(param_modify_host,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro["io"] = param_modify_host['-io']
                    enviro["priority"] = param_modify_host['-pry']                   
                    if param_modify_host['-au'] == '1':
                        enviro["auto_start"] = True
                    else:
                        enviro["auto_start"] = False
                    if param_modify_host['-bk'] == '1':
                        enviro["backup"] = True
                    else:
                        enviro["backup"] = False
                    if param_modify_host['-usb'] == '1':
                        enviro["usb"] = True
                    else:
                        enviro["usb"] = False
    ...
    *************************************************
        ...
        if param_image(param_query_forwarder,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["type"] = param_query_forwarder['-type']
                    enviro["target"] = param_query_forwarder['-t']                  
    ...
    def do_query_forwarder_group(self,argc):
        argc = argc.split()
        if param_image(param_query_forwarder_group,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_forwarder_group", query_forwarder_group))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["pool"] = param_query_forwarder_group['-p'] 
    ...
    def do_create_forwarder_group(self,argc):
        argc = argc.split()
        if param_image(param_create_forwarder_group,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("create_forwarder_group", create_forwarder_group))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["pool"] = param_create_forwarder_group['-p']
                    enviro["count"] = param_create_forwarder_group['-c']
    ...
    def do_delete_forwarder_group(self,argc):
        argc = argc.split()
        if param_image(param_delete_forwarder_group,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("delete_forwarder_group", delete_forwarder_group))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_delete_forwarder_group['-id']
                    enviro["option"] = param_delete_forwarder_group['-p']
    ...
    def do_attach_forwarder(self,argc):
        argc = argc.split()
        if param_image(param_attach_forwarder,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("attach_forwarder", attach_forwarder))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_attach_forwarder['-id']
                    enviro["type"] = param_attach_forwarder['-type']
                    enviro["target"] = param_attach_forwarder['-t']
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_detach_forwarder(self,argc):
        argc = argc.split()
        if param_image(param_detach_forwarder,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("detach_forwarder", detach_forwarder))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_detach_forwarder['-id']
                    enviro["type"] = param_detach_forwarder['-type']
                    enviro["target"] = param_detach_forwarder['-t']
                    enviro["group"] = param_detach_forwarder['-g']
    ...
    def do_migrate_forwarder(self,argc):
        argc = argc.split()
        if param_image(param_migrate_forwarder,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("migrate_forwarder", migrate_forwarder))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_migrate_forwarder['-id']
                    enviro["ip"] = param_migrate_forwarder['-ip']
                    enviro["target"] = param_migrate_forwarder['-t']
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_delete_forwarder(self,argc):
        argc = argc.split()
        if param_image(param_delete_forwarder,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("delete_forwarder", delete_forwarder))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_delete_forwarder['-id']
                    enviro["target"] = param_delete_forwarder['-t']
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_create_forwarder(self,argc):
        argc = argc.split()
        if param_image(param_create_forwarder,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("create_forwarder", create_forwarder))

                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_create_forwarder['-id']
                    enviro["count"] = param_create_forwarder['-c']
    ...
    **************************************************
        ...
        if param_image(param_monitor_heart_beat,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["task"] = param_monitor_heart_beat['-id']
        ...
        if param_image(param_monitor_data,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["task"] = param_monitor_data['-id']
        ...
        if param_image(param_attach_disk,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    enviro["path"] = param_attach_disk['-path']
                    enviro["crypt"] = param_attach_disk['-crypt']
                    #start Test
                    service.beginTest()
        ...
        if param_image(param_modify_device,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    ...
                    #optional parameter:
                    if param_modify_device['-a'] == '1':
                        enviro["authen"] = True
                    else:
                        enviro["authen"] = False

                    if param_modify_device['-et'] == '1':
                        enviro["crypt_trans"] = True
                    else:
                        enviro["crypt_trans"] = False

                    if param_modify_device['-ct'] == '1':
                        enviro["cmp_trans"] = True
                    else:
                        enviro["cmp_trans"] = False
    ...
    def do_create_snapshot(self,argc):
        """
        creat a sanpshot for a device.
        
        ************** create_snapshot ***********
         -name(*): name of the snapshot.
         -tid(*):  target device uuid
        **************************************************
        eg: eg:create_snapshot -name snapshot-001 -tid c9ebbe4db3b14bfe8060d922495b09fe 
        """
        argc = argc.split()
        if param_image(param_create_snapshot,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("create_snapshot", create_snapshot))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_name"] = param_create_snapshot['-name']
                    enviro["target_device_uuid"] = param_create_snapshot['-tid']
    ...
    def do_delete_snapshot(self,argc):
        """
        delete a sanpshot.
        ...
        ************** delete_snapshot ***********
         -id(*):  snapshot uuid
        **************************************************
        eg: eg:delete_snapshot  -id c9ebbe4db3b14bfe8060d922495b09fe 
        """
        argc = argc.split()
        if param_image(param_delete_snapshot,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("delete_snapshot", delete_snapshot))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_uuid"] = param_delete_snapshot['-id']
    ...
    def do_resume_snapshot(self,argc):
        """
        resume a sanpshot.
        ...
        ************** resume_snapshot ***********
         -id(*):  snapshot uuid
        **************************************************
        eg: eg:resume_snapshot  -id c9ebbe4db3b14bfe8060d922495b09fe 
        """
        argc = argc.split()
        if param_image(param_resume_snapshot,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("resume_snapshot", resume_snapshot))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_uuid"] = param_resume_snapshot['-id']
    ...
    def do_query_snapshot(self,argc):
        """
        query  sanpshot of device.
        
        ************** query_snapshot ***********
         -tid(*):  target device uuid
        **************************************************
        eg: eg:query_snapshot  -tid c9ebbe4db3b14bfe8060d922495b09fe 
        """
        argc = argc.split()
        if param_image(param_query_snapshot,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_snapshot", query_snapshot))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["target_device_uuid"] = param_query_snapshot['-tid']
    ...
    def do_query_snapshot_pool(self,argc):
        """
        query a snapshot pool with specific use.    
    ************** query_snapshot_pool ***********
    None
    **************************************************
        eg:query_snapshot_pool
        """
        argc = argc.split()
        try:
            global service
            ...
            finished_event = threading.Event()
            if service != None:               
                service.case_manager.addTestCase(TestCase("query_snapshot_pool", query_snapshot_pool))
                enviro = service.case_manager.getParam()
                enviro["control_server"] = service.zc_node_name
                service.beginTest()
                finished_event.wait()
    ...
    def do_modify_snapshot_pool(self,argc):
        """
        ...
        if param_image(param_modify_storage_pool,argc) == CHECK_PASS:
            try:
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("modify_snapshot_pool", modify_snapshot_pool))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_pool_id"] = param_modify_storage_pool['-id']
                    enviro["snapshot_pool_name"] = param_modify_storage_pool['-n']
    ...
    def do_create_snapshot_pool(self,argc):
        #creat a snapshot_pool with specific use.
        ...
        argc = argc.split()
        if param_image(param_create_snapshot_pool,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("create_snapshot_pool", create_snapshot_pool))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_pool_name"] = param_create_snapshot_pool['-n']
                    service.beginTest()
                    #wait threading
                    finished_event.wait()
                    finished_event.clear()
    ...
    def do_delete_snapshot_pool(self,argc):
        #delete an existed snapshot pool.
        #******:delete_snapshot_pool param*******
         #-id(*) target snapshot pool uuid
        #***********************************
        #eg::delete_snapshot_pool -id xxx
        argc = argc.split()
        if param_image(param_delete_snapshot_pool,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("delete_snapshot_pool", delete_snapshot_pool))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_pool_id"] = param_delete_snapshot_pool['-id']
    ...
    def do_query_snapshot_node(self,argc):
        """
        ...
        ***********************************
        ...
        """
        argc = argc.split()
        if param_image(param_query_snapshot_node,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_snapshot_node", query_snapshot_node))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_pool_id"] = param_query_storage_resource['-p']
    ...
    def do_add_snapshot_node(self,argc):
        #add the snapshot node to pool.
        ...
        argc = argc.split()
        if param_image(param_add_snapshot_node,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("add_snapshot_node", add_snapshot_node))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_pool_id"] = param_add_snapshot_node['-p']
                    enviro["name"] = param_add_snapshot_node['-n']
    ...
    def do_remove_snapshot_node(self,argc):
        #remove the snapshot node to pool.
        ...
        argc = argc.split()
        if param_image(param_remove_snapshot_node,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event.clear()
                ...
                if service != None:
                    #add TestCase to CaseManager
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("remove_snapshot_node", remove_snapshot_node))
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["snapshot_poll_id"] = param_remove_snapshot_node['-p']
                    enviro["name"] = param_remove_snapshot_node['-n']
    ...
    def do_flush_disk_image(self,argc):
        argc = argc.split()
        if param_image(param_flush_disk_image,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("flush_disk_image", flush_disk_image))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_flush_disk_image['-id']
                    enviro["disk"] = param_flush_disk_image['-dt']
                    enviro["mode"] = param_flush_disk_image['-mode']
                    enviro["image"] = param_flush_disk_image['-md']
                    service.beginTest()
    ...
    def do_backup_host(self,argc):
        
        argc = argc.split()
        if param_image(param_backup_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("backup_host", backup_host))                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_backup_host['-id']
                    enviro["mode"] = param_backup_host['-md']
                    enviro["disk"] = param_backup_host['-dt']
                    if param_backup_host['-md'] == '1':
                        enviro["disk"] = param_backup_host['-dt']
                    else:
                        enviro["disk"] = False
                    service.beginTest()                        
    ...
    def do_resume_host(self,argc):
        
        argc = argc.split()
        if param_image(param_resume_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("resume_host", resume_host))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_resume_host['-id']
                    enviro["mode"] = param_resume_host['-md']
                    enviro["disk"] = param_resume_host['-dt']
                    if param_resume_host['-md'] == '1':
                        enviro["disk"] = param_resume_host['-dt']
                    else:
                        enviro["disk"] = False
    ...
    def do_query_host_backup(self,argc):
    
        argc = argc.split()
        if param_image(param_query_host_backup,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_host_backup", query_host_backup))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["uuid"] = param_query_host_backup['-id']
                    service.beginTest()
    ...
    def do_add_rule(self,argc):
    
        argc = argc.split()
        if param_image(param_add_rule,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("add_rule", add_rule))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["target"] = param_add_rule['-target']
                    enviro["mode"] = param_add_rule['-mode']
                    enviro["ip"] = param_add_rule['-ip']
                    enviro["port"] = param_add_rule['-port']
                    service.beginTest()
    ...
    def do_remove_rule(self,argc):
   
       argc = argc.split()
       if param_image(param_remove_rule,argc) == CHECK_PASS:
           try:
               global service
               global finished_event                   
               finished_event = threading.Event()
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("remove_rule", remove_rule))
                       
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   enviro["target"] = param_remove_rule['-target']
                   enviro["mode"] = param_remove_rule['-mode']
                   enviro["ip"] = param_remove_rule['-ip']
                   enviro["port"] = param_remove_rule['-port']
                   service.beginTest()
    ...
    def do_query_rule(self,argc):
   
       argc = argc.split()
       if param_image(param_query_rule,argc) == CHECK_PASS:
           try:
               global service
               global finished_event                   
               finished_event = threading.Event()
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("query_rule", query_rule))
                       
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   enviro["target"] = param_query_rule['-t']  
                   service.beginTest()
    ...
    def do_query_network(self,argc):
         ...
         #eg:query_network
            try:
                ...
                finished_event = threading.Event()
                if service != None:               
                    service.case_manager.addTestCase(TestCase("query_network", query_network))
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    service.beginTest()
                    finished_event.wait()
    ...
    def do_create_network(self,argc):       
        argc = argc.split()
        if param_image(param_create_network,argc) == CHECK_PASS:
           try:
               global service
               global finished_event
               finished_event = threading.Event()
               if service != None:
                   service.case_manager.clearTestCase()
                   service.case_manager.addTestCase(TestCase("create_network", create_network))
                   
                   #parameter initail and begin test.
                   enviro = service.case_manager.getParam()
                   enviro["control_server"] = service.zc_node_name
                   #essential parameter 
                   enviro["name"] = param_create_network['-n']
                   enviro["pool"] = param_create_network['-p']
                   enviro["netmask"] = param_create_network['-ns']
                   enviro["description"] = param_create_network['-des']
                   #start Test
                   service.beginTest()                   
                   finished_event.wait()
                   finished_event.clear()
               ...
               print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_modify_network(self,argc):
        argc = argc.split()
        if param_image(param_modify_network,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("modify_network", modify_network))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["uuid"] = param_modify_network['-id']
                    enviro["pool"] = param_modify_network['-p']
                    enviro["name"] = param_modify_network['-n']
                    enviro["description"] = param_modify_network['-des']
                    enviro["ip"] = param_modify_network['-ip']
                    enviro["option"] = param_modify_network['-op']
                    service.beginTest()
                    finished_event.wait()
                    finished_event.clear()
                    service.case_manager.enviro.clear()
    ...
    def do_query_network_detail(self,argc): 
        argc = argc.split()
        if param_image(param_query_network_detail,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_network_detail", query_network_detail))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["id"] = param_query_network_detail['-id']
    ...
    def do_start_network(self,argc):      
        argc = argc.split()
        if param_image(param_start_network,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("start_network", start_network))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_start_network['-id']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_stop_network(self,argc):      
        argc = argc.split()
        if param_image(param_stop_network,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("stop_network", stop_network))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_stop_network['-id']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_delete_network(self,argc):      
        argc = argc.split()
        if param_image(param_delete_network,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("delete_network", delete_network))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_delete_network['-id']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_query_network_host(self,argc):      
        argc = argc.split()
        if param_image(param_query_network_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_network_host", query_network_host))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_query_network_host['-id']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_attach_host(self,argc):      
        argc = argc.split()
        if param_image(param_attach_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("attach_host", attach_host))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_attach_host['-id']
                    enviro["host"] = param_attach_host['-host']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_detach_host(self,argc):      
        argc = argc.split()
        if param_image(param_detach_host,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("detach_host", detach_host))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_detach_host['-id']
                    enviro["host"] = param_detach_host['-host']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_network_attach_address(self,argc):      
        argc = argc.split()
        if param_image(param_network_attach_address,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("network_attach_address", network_attach_address))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_network_attach_address['-id']
                    enviro["count"] = param_network_attach_address['-count']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_network_detach_address(self,argc):      
        argc = argc.split()
        if param_image(param_network_detach_address,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("network_detach_address", network_detach_address))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_network_detach_address['-id']
                    enviro["ip"] = param_network_detach_address['-ip']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now. Error Info:%s" % str(e)
    def do_network_bind_port(self,argc):      
        argc = argc.split()
        if param_image(param_network_bind_port,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("network_bind_port", network_bind_port))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_network_bind_port['-id']
                    enviro["port"] = param_network_bind_port['-port']
                    #start test.
                    service.beginTest()
                ...
                print "Execute fail, goto except now.", e
    def do_network_unbind_port(self,argc):      
        argc = argc.split()
        if param_image(param_network_unbind_port,argc) == CHECK_PASS:
            try:
                global service
                global finished_event                   
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("network_unbind_port", network_unbind_port))
                        
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    enviro["id"] = param_network_unbind_port['-id']
                    enviro["port"] = param_network_unbind_port['-port']
                    #start test.
                    service.beginTest()
    ...
    def do_query_operate_detail(self,argc):
        argc = argc.split()
        if param_image(param_query_operate_detail,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_operate_detail", query_operate_detail))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["target"] = param_query_operate_detail['-t']
                    enviro["level"] = param_query_operate_detail['-l']
                    enviro["begin"] = param_query_operate_detail['-begin']
                    enviro["end"] = param_query_operate_detail['-end']
    ...
    def do_query_service_detail(self,argc):
        argc = argc.split()
        if param_image(param_query_service_detail,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_service_detail", query_service_detail))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["target"] = param_query_service_detail['-t']
                    enviro["level"] = param_query_service_detail['-l']
                    enviro["begin"] = param_query_service_detail['-begin']
                    enviro["end"] = param_query_service_detail['-end']
    ...
    def do_query_operate_summary(self,argc):
        argc = argc.split()
        if param_image(param_query_operate_summary,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_operate_summary", query_operate_summary))
                    
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["target"] = param_query_operate_summary['-t']
                    enviro["begin"] = param_query_operate_summary['-begin']
                    enviro["end"] = param_query_operate_summary['-end']
    ...
    def do_query_service_summary(self,argc):
        argc = argc.split()
        if param_image(param_query_service_summary,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_service_summary", query_service_summary))
                   
                    #parameter initail and begin test.
                    enviro = service.case_manager.getParam()
                    enviro["control_server"] = service.zc_node_name
                    #essential parameter 
                    enviro["target"] = param_query_service_summary['-t']
                    enviro["begin"] = param_query_service_summary['-begin']
                    enviro["end"] = param_query_service_summary['-end']
    ...
    def do_query_storage_device(self,argc):
        argc = argc.split()
        if param_image(param_query_storage_device,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("query_storage_device", query_storage_device))
                    ...
                    #parameter initail and begin test.
                    enviro["level"] = param_query_storage_device['-l']
                    enviro["target"] = param_query_storage_device['-t']
                    enviro["disk_type"] = param_query_storage_device['-d']
    ...
    def do_add_storage_device(self,argc):
        argc = argc.split()
        if param_image(param_add_storage_device,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("add_storage_device", add_storage_device))
                    ...
                    #parameter initail and begin test.
                    enviro["level"] = param_add_storage_device['-l']
                    enviro["target"] = param_add_storage_device['-t']
                    enviro["disk_type"] = param_add_storage_device['-d']
                    enviro["index"] = param_add_storage_device['-i']
                    enviro["name"] = param_add_storage_device['-n']
                    enviro["path"] = param_add_storage_device['-p']
                    enviro["crypt"] = param_add_storage_device['-c']
                ...
                print "Execute fail, goto except now.",e
    def do_remove_storage_device(self,argc):
        argc = argc.split()
        if param_image(param_remove_storage_device,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("remove_storage_device", remove_storage_device))
                    ...
                    #parameter initail and begin test.
                    enviro["level"] = param_remove_storage_device['-l']
                    enviro["target"] = param_remove_storage_device['-t']
                    enviro["disk_type"] = param_remove_storage_device['-d']
                    enviro["index"] = param_remove_storage_device['-i']
                ...
                print "Execute fail, goto except now.",e
    def do_enable_storage_device(self,argc):
        argc = argc.split()
        if param_image(param_enable_storage_device,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("enable_storage_device",enable_storage_device ))
                    ...
                    #parameter initail and begin test.
                    enviro["level"] = param_enable_storage_device['-l']
                    enviro["target"] = param_enable_storage_device['-t']
                    enviro["disk_type"] = param_enable_storage_device['-d']
                    enviro["index"] = param_enable_storage_device['-i']
                ...
                print "Execute fail, goto except now.",e
    def do_disable_storage_device(self,argc):
        argc = argc.split()
        if param_image(param_disable_storage_device,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("disable_storage_device",disable_storage_device ))
                    ...
                    #parameter initail and begin test.
                    enviro["level"] = param_disable_storage_device['-l']
                    enviro["target"] = param_disable_storage_device['-t']
                    enviro["disk_type"] = param_disable_storage_device['-d']
                    enviro["index"] = param_disable_storage_device['-i']
    ...
    def do_modify_service(self,argc):
        argc = argc.split()
        if param_image(param_modify_service,argc) == CHECK_PASS:
            try:
                global service
                global finished_event
                finished_event = threading.Event()
                if service != None:
                    service.case_manager.clearTestCase()
                    service.case_manager.addTestCase(TestCase("modify_service",modify_service ))
                    ...
                    #parameter initail and begin test.
                    enviro["type"] = param_modify_service['-t']
                    enviro["target"] = param_modify_service['-name']
                    enviro["disk_type"] = param_modify_service['-dt']
                    enviro["disk_source"] = param_modify_service['-ds']
                    enviro["crypt"] = param_modify_service['-c']
...
if __name__ == "__main__":
    ...
    #welcome = 'Welcome to zhicloud manage platform cli tool(v1.20).\n Type "help" for more information.'
