...
class DeleteForwarderTask(BaseTask):
    operate_timeout = 20
    def __init__(self, task_type, messsage_handler,
                 case_manager,logger_name):
        self.case_manager = case_manager
        #logger_name = "task.add_storage_device"
        BaseTask.__init__(self, task_type, RequestDefine.delete_forwarder,
                          messsage_handler, logger_name)
        
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.delete_forwarder, result_success,
                             self.onDeleteSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.delete_forwarder, result_fail,
                             self.onDeleteFail)
    ...
    def invokeSession(self, session):
        ...
        """
        request = getRequest(RequestDefine.delete_forwarder)
        param = self.case_manager.getParam()
        control_server = param["control_server"]
        uuid = param["uuid"]
        request.setString(ParamKeyDefine.uuid,uuid)
        list = param["target"].split(',')
        target = []
        for unit in list:
            target.append(int(unit))
        
        request.setUIntArray(ParamKeyDefine.target, target)
