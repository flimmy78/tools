...
class QuerySnapshotPoolTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler,
        ...
        BaseTask.__init__(self, task_type, RequestDefine.query_snapshot_pool,
                          messsage_handler, logger_name)
        
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.query_snapshot_pool, result_success,
                             self.onQuerySuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.query_snapshot_pool, result_fail,
                             self.onQueryFail)
    ...
    def invokeSession(self, session):
        ...
        """        
        request = getRequest(RequestDefine.query_snapshot_pool)
...
##                       (session.session_id, control_server))
    ...
    def onQuerySuccess(self, msg, session):
        ...
        snapshot = msg.getUIntArrayArray(ParamKeyDefine.snapshot)
        ...
        count = len(uuid)
        self.info("[%08X]query snapshot pool success, %d pool(s) available"%
                       (session.session_id, count))

        #show query result
        newstatus = ChangeResuleStatus(status,Status_snapshot_pool)
        title = ['Name','UUID','snapshot','Status']
        print_test_result(title,name,uuid,snapshot,newstatus)
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query snapshot pool FAIL"%
                  (session.session_id))
