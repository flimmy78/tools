...
class ResumeSnapShotTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler,
        ...
        BaseTask.__init__(self, task_type, RequestDefine.resume_snapshot,
                          messsage_handler, logger_name)
        
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.resume_snapshot, result_success,
                             self.onCreateSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.resume_snapshot, result_fail,
                             self.onCreateFail)
    ...
    def invokeSession(self, session):
        ...
        """
        request = getRequest(RequestDefine.resume_snapshot)
        param = self.case_manager.getParam()
        control_server = param["control_server"]       
        snapshot_uuid = param["snapshot_uuid"]
        request.setString(ParamKeyDefine.uuid, snapshot_uuid)
       
        self.info("[%08X]request resume snapshot '%s' to control server '%s'"%
    ...
    def onCreateSuccess(self, msg, session):
        ...
        self.info("[%08X]resume snapshot success"%
    ...
    def onCreateFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]resume snapshot fail"%
                  (session.session_id))
    ...
    def onCreateTimeout(self, msg, session):
        self.info("[%08X]resume snapshot timeout"%
                  (session.session_id))
