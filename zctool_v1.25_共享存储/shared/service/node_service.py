...
class NodeService(BaseService):
    ...
    def __init__(self,
        ...
        self.timer.bindEventHandler(self.sendMessageListToSelf)
    ...
    """
    put message list into queue tail
    @message_list:message_list
    @sender:sender name
    @return:True - put success
    """
    def putMessageList(self, message_list, sender):
        request_list = []
        for msg in message_list:
            event = DispatchEvent()
            event.message = msg
            event.sender = sender
            request_list.append(event)
        return self.putRequestList(request_list)
...
##            receiver, node.session))
    ...
    def sendMessageListToSelf(self, msg_list):
        return self.putMessageList(msg_list, self.node)
...
##        self.info("<NodeService>node allocated, name '%s', session [%08X]"%(node_name, session_id))
    ...
    def deallocateNode(self, node_name):
        if self.node_map.has_key(node_name):
            entry = self.node_map[node_name]
            if 0 != entry.session:
                self.transport.disconnect(entry.session)
...
##            self.info("<NodeService>node deallocated, name '%s', session [%08X]"%(entry.name, entry.session))
    ...
    def connectRemoteNode(self, name, node_type, ip, port, group = "default"):
        ...
        self.transport.connect(name, ip, port)
...
##        self.info("<NodeService>receive message from sender '%s', id %d"%(sender, message.id))
    ...
    def __isSystemMessage(self, message):
        if message.type == AppMessage.EVENT:
            ...
            if message.id == EventDefine.channel_connected:
                ...
                return True
            elif message.id == EventDefine.service_update:
                return True
    ...
    def __handleSystemMessage(self, message, sender):
        if message.type == AppMessage.EVENT:
            ...
            if message.id == EventDefine.channel_connected:
                ...
                return self.__handleServiceUpdate(message, sender)
    ...
    def __handleServiceUpdate(self, message, sender):
        ##success
        service_count = getUInt(message, ParamKeyDefine.count)
        
        if 0 == service_count:
##            self.info("<NodeService>debug:no service update")
            return
        name_list = message.getStringArray(ParamKeyDefine.name)
        type_list = message.getUIntArray(ParamKeyDefine.type)
        group_list = message.getStringArray(ParamKeyDefine.group)
        ip_list = message.getStringArray(ParamKeyDefine.ip)
        port_list = message.getUIntArray(ParamKeyDefine.port)
        version = message.getStringArray(ParamKeyDefine.version)        

        for index in range(service_count):
            service_name = name_list[index]
            if not self.node_map.has_key(service_name):
                service_type = type_list[index]
                remote_ip = ip_list[index]
                remote_port = port_list[index]
                service_group = group_list[index]
                self.info("<NodeService>reconnect service '%s', type %d, address '%s:%d'..."%(
                    service_name, service_type,
                    remote_ip, remote_port))                
                self.connectRemoteNode(service_name, service_type,
                                       remote_ip, remote_port,
                                       service_group)
##            else:
##                self.info("<NodeService>debug:ignore service update, service '%s' already connected"%(
##                    service_name))
    ...
    def __onJoinDomainResponse(self, response, sender):
        ...
        group_list = getStringArray(response, ParamKeyDefine.catalog)
...
##                self.timer.stop()
    ...
    def __onCheckService(self):
        self.info("<NodeService>check domain service...")
        self.domain_service.query()
...
##    client_ip = "192.168.66.102"
    ...
    if len(sys.argv) > 1:
        ...
        ##client
        client = NodeService(NodeTypeDefine.manage_terminal, "data_driver", "akumas_test", client_ip)
        client.start()
        client.connectRemoteNode("data_server_00505629b9dc", NodeTypeDefine.data_server, server_ip, server_port)
        time.sleep(10)
        client.stop()      
