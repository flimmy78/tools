...
##                       (session.session_id, control_server))
    ...
    def onQuerySuccess(self, msg, session):
        ...
        #file_type = msg.getUIntArray(ParamKeyDefine.file_type)
        ...
        #title = ['Disk Image Name','UUID','Size(MB)','Status','Desc','file_type','identity']
        #print_test_result(title,name,uuid,newsize,newstatus,description,file_type,identity)
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query disk image fail"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query disk image timeout"%
                  (session.session_id))
