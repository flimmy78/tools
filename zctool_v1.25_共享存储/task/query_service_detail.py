...
class QueryServiceDetailTask(BaseTask):
    ...
    def invokeSession(self, session):
        ...
        """
        request = getRequest(RequestDefine.query_service_detail)
    ...
    def onQuerySuccess(self, msg, session):
        ...
        value = []
        title.append("uuid")
        value.append(msg.getString(ParamKeyDefine.uuid))
        title.append("name")
        ...
        status = ChangeResuleStatus(status,Status_server)
        title.append("status")
        value.append(status)       
        ...
        self.info("[%08X]query service detail success" % (session.session_id))
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query service detail fail"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query service detail timeout"%
                  (session.session_id))
