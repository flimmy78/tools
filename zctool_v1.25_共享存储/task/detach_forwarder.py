...
class DetachForwarderTask(BaseTask):
    operate_timeout = 20
    def __init__(self, task_type, messsage_handler,
                 case_manager,logger_name):
        self.case_manager = case_manager
        #logger_name = "task.add_storage_device"
        BaseTask.__init__(self, task_type, RequestDefine.detach_forwarder,
                          messsage_handler, logger_name)
        
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.detach_forwarder, result_success,
                             self.onCreateSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE,
                             RequestDefine.detach_forwarder, result_fail,
                             self.onCreateFail)
    ...
    def invokeSession(self, session):
        ...
        """
        request = getRequest(RequestDefine.detach_forwarder)
    ...
    def onCreateSuccess(self, msg, session):
        ...
        self.info("[%08X]detach forwarder success"%
    ...
    def onCreateFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]detach forwarder fail"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onCreateTimeout(self, msg, session):
        self.info("[%08X]detach forwarder tiemout"%
                  (session.session_id))
