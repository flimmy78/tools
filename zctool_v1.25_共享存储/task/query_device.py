...
class QueryDeviceTask(BaseTask):
    ...
    def onQuerySuccess(self, msg, session):
        ...
        identity = msg.getStringArray(ParamKeyDefine.identity)
	...
	#print "identity is %s" % identity
        
        newdisk=[]
        for disksize in disk_volume:
            newdisk.append(Change_Bit_to_Mb(disksize))
        newpage=Change_Bit_to_Kb(page)
            
        count = len(uuid)
        self.info("[%08X]query device success, %d device(s) available"%
                       (session.session_id, count))

        title = ['Device Name','UUID','Disk(MiB)','Page(KiB)','level','Security','crypt','Status']
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query device fail"%
                  (session.session_id))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query device timeout"%
                  (session.session_id))
